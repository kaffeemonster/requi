...
import isa_shell
from isa_shell import ShellCPU, ternlog_w, permb_w, bitfrob_w, ctrl_w
...
for trial in range(25):
    ...
    cpu.load_words(0x1080, [ternlog_w(lut, 5, 1, 2, 3), ctrl_w(0, 0, 0, 0, scale=0, wrf=False)])
    cpu.run([0], start=0x1080)
    ...
    cpu = ShellCPU()
    cpu.write_dst(1, a); cpu.write_dst(2, b); cpu.write_dst(3, c)
    cpu.load_words(0x1080, [permb_w(0, 6, 1, 2, 3), ctrl_w(0, 0, 0, 0, scale=0, wrf=False)])
    cpu.run([0], start=0x1080)
    r_ref = permb(a, b, c, 0, False, 0, 0, blank_enable=False)['res']
    if cpu.read_dst(6) != r_ref:
        fails += 1; print(f"PERMB fail: shell={cpu.read_dst(6):08x} ref={r_ref:08x}")
    # --- bitfrob LZC ---
    cpu = ShellCPU()
    cpu.write_dst(1, a)
    cpu.load_words(0x1080, [bitfrob_w(BitFrobMode.LZC, 7, 1, 0), ctrl_w(0, 0, 0, 0, scale=0, wrf=False)])
    cpu.run([0], start=0x1080)
